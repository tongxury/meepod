...
func RSASignWithRSA1(signContent string, privateKey string) (string, error) {
	priKey, err := parsePrivateKey(privateKey)
	if err != nil {
		return "", err
	}
	hashed := sha1.Sum([]byte(signContent))
	// 使用RSA私钥进行签名，注意参数 crypto.SHA256
	signature, err := rsa.SignPKCS1v15(rand.Reader, priKey, crypto.SHA1, hashed[:])
	if err != nil {
		return "", err
	}
...
func RSASignWithRSA2(signContent string, privateKey string) (string, error) {
	priKey, err := parsePrivateKey(privateKey)
	if err != nil {
		return "", err
	}
