...
func NewClient() *Client {
	return &Client{
		...
		rateLimiter: ratelimit.New(1, ratelimit.Per(15*time.Second)),
	}
}
...
func (c *Client) GetCoinOhlcs(ctx context.Context, id string, days int64) (Ohlcs, error) {
	url := fmt.Sprintf("%s/api/v3/coins/%s/ohlc?vs_currency=usd&days=%d", c.base, id, days)
