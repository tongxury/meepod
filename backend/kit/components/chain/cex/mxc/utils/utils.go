...
// 公共get请求
func PublicGet(urlStr string, jsonParams string) (interface{}, error) {
	var path string
	if jsonParams == "" {
		path = urlStr
	} else {
...
// 私有get请求
func PrivateGet(urlStr string, jsonParams string) (interface{}, error) {
	var path string
...
// 私有post请求
func PrivatePost(urlStr string, jsonParams string) (interface{}, error) {
	var path string
	timestamp := time.Now().UnixNano() / 1e6
	fmt.Println(timestamp)
	if jsonParams == "" {
...
// 私有delete请求
func PrivateDelete(urlStr string, jsonParams string) (interface{}, error) {
	var path string
	timestamp := time.Now().UnixNano() / 1e6
	fmt.Println(timestamp)
	if jsonParams == "" {
