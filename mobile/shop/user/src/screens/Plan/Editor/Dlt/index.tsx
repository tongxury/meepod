...
import {cnm, factorial} from "../../../../utils/math";
...
const DltSelectorView = ({onConfirm}: {
    onConfirm: (plan: Ticket) => void,
}) => {

    const {settingsState: {settings}} = useContext<any>(AppContext);
    const maxBlue = settings.dltBlueLimit
    const maxRed = settings.dltRedLimit
    const maxDRed = settings.dltDRedLimit
    ...
    const updateAmount = () => {
        ...
        if (redCount + doubleRedCount < 5 || blueCount < 2) {
            setAmount(0)
            return
        }


        const num = cnm(redCount, 5 - doubleRedCount) * cnm(blueCount, 2)
    ...
    const validPlan = () => {
        return selectedRed.length + doubleSelectedRed.length >= 5 && selectedBlue.length >= 2
    }
...
export default DltSelectorView