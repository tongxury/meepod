...
const useHook = () => {
    ...
    const redOptions = [...Array(35).keys()].map(t => t + 1).map(t => {
        const v = t <= 9 ? `0${t}` : `${t}`
        return {value: v, label: v}
    })
    const blueOptions = [...Array(12).keys()].map(t => t + 1).map(t => {
        const v = t <= 9 ? `0${t}` : `${t}`
        return {value: v, label: v}
    })
    ...
    const getRandom = (): Ticket => {
        const randomRed = [...redOptions].sort(() => Math.random() - 0.5).slice(0, 5)
        const randomBlue = [...blueOptions].sort(() => Math.random() - 0.5).slice(0, 2)
