...
import {payProxyReward} from "../../../../service/api";

const PayTrigger = ({id, onConfirm, style}: {
    id: string,
    onConfirm?: () => void,
    ...
    const confirm = () => {
        payProxyReward({rewardId: id}).then(rsp => {
            setVisible(false)
            onConfirm?.()
        })
    }
    ...
    return <View style={style}>
        <RneButton onPress={() => setVisible(true)} >结算</RneButton>
        <Modal
